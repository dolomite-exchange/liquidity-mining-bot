...
export interface BorrowRebatesPerNetworkCalculation {
  epoch: number;
  merkleRoot: string | null
}

export async function calculateBorrowRebatesPerNetwork(
  epoch: number = parseInt(process.env.EPOCH_NUMBER ?? 'NaN', 10),
): Promise<BorrowRebatesPerNetworkCalculation> {
...
if (isScript()) {
  calculateBorrowRebatesPerNetwork()
    .then(() => {
